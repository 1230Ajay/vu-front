...
class ChatMessage {
   ...
   ChatMessage({this.uid,this.chatId,this.recipientId,this.senderId,this.content,this.time_stamp});
   ...
   factory ChatMessage.fromJson(Map<String,dynamic> data){
     return ChatMessage(
       ...
       time_stamp: DateTime.parse(data["timestamp"]),
     );
   }
...
ChatMessageRes({this.status,this.messages});
factory ChatMessageRes.fromJson(int status ,Map<String,dynamic> data){
  return ChatMessageRes(
    status: status,
    messages: data ==null ?[]:List<ChatMessage>.from(data["data"].map((e) => ChatMessage.fromJson(e)))
  );
}
}
